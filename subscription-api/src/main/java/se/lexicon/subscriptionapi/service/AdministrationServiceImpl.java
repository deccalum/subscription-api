...
public class AdministrationServiceImpl implements AdministrationService {
    private final RequestRepository changeRequestRepository;
    ...
    public ChangeRequestResponse approveRequest(Long id, String adminEmail) {
        return Optional.ofNullable(id)
                ...
                .flatMap(request -> resolveAdmin(adminEmail).map(admin -> requestMapper.toApproved(request, admin)))
                .map(changeRequestRepository::save)
                .map(requestMapper::toResponse)
                .orElseThrow(
                        () -> Optional.ofNullable(id).flatMap(changeRequestRepository::findById).isEmpty()
    ...
    public ChangeRequestResponse rejectRequest(Long id, String reason, String adminEmail) {
        return Optional.ofNullable(id)
                .flatMap(changeRequestRepository::findById)
                .flatMap(request -> resolveAdmin(adminEmail)
                        .map(admin -> requestMapper.toRejected(request, reason, admin)))
                .map(changeRequestRepository::save)
                .map(requestMapper::toResponse)
                .orElseThrow(() -> Optional.ofNullable(id).flatMap(changeRequestRepository::findById).isEmpty()
                        ? new ResourceNotFoundException("auto")
                        : new BusinessRuleException("auto"));
    }
    ...
    private Optional<UserAdmin> resolveAdmin(String email) {
        return Optional.ofNullable(email).flatMap(userRepository::findByEmail).filter(UserAdmin.class::isInstance)
                .map(UserAdmin.class::cast);
    }
    ...
    private Optional<ChangeRequest> executeDeletePlan(ChangeRequest request) {
        return Optional.ofNullable(request).filter(DeletePlanRequest.class::isInstance)
                .map(DeletePlanRequest.class::cast).map(r -> {
